...
public class WorldRegistryImpl extends UnicastRemoteObject implements WorldRegistry {
    ...
    public boolean createHideout(int map, int guildid) {
        MapleGuild g = guilds.get(guildid);
        if (g != null) {
            return g.createHideout(map);
        }
        return false;
    }

    public boolean extendHideoutTime(int guildid) {
        MapleGuild g = guilds.get(guildid);
        if (g != null) {
            return g.extendHideoutTime();
        }
        return false;
    }

    public void removeHideout(int guildid) {
        MapleGuild g = guilds.get(guildid);
        if (g != null) {
            g.removeHideout();
        }
