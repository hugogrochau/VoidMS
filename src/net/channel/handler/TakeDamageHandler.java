    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (damagefrom == -2) {
            ...
            if (MapleLifeFactory.getMonster(monsteridfrom) == null) {
                c.getPlayer().ban("Autoban '" + c.getPlayer().getName() + "': Packet Editing (taking damage from an invalid monster)", true);
                return;
            }
            oid = slea.readInt();
        ...
        if (monsteridfrom == 9400551 && !player.isHidden()) {
            player.setHp(0);
        ...
        if (damage > 0) {
            ...
            if (!player.isHidden() && player.isAlive()) {
                ...
                if (player.getMap().getId() == 260000201 && monsteridfrom == 9300166) {
                    player.getMap().broadcastMessage(MaplePacketCreator.sendYellowTip("[BOMBERMAN] BOOM! " + player.getName() + " has just been blown up!"));
