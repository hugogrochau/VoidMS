        ...
        switch (mode) {
            case 0:
                ...
                if (!bEdit) {
                    ...
                    c.getPlayer().getGuild().guildMessage(MaplePacketCreator.serverNotice(5, "[Guild] New BBS by " + c.getPlayer().getName() + ": " + title));
                } else {
                    editBBSThread(c, title, text, icon, localthreadid);
                    c.getPlayer().getGuild().guildMessage(MaplePacketCreator.serverNotice(5, "[Guild] " + c.getPlayer().getName() + " has just edited the BBS: " + title));
                }
                break;
