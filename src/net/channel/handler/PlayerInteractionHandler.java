    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) { // Trade
                if (c.getPlayer().getPets().length > 0) {
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    if (c.getPlayer().getInventory(MapleInventoryType.CASH).countById(itemId) <= 0) {
                        return;
                    }
                        ...
                        if (ips.getShopType() == 2) {
                            ...
                            for (MaplePlayerShopItem items : ips.getItems()) {
                                if (items.getBundles() > 0) {
                                    ...
                                    item.setQuantity((short) (item.getQuantity() * items.getBundles()));
                                    if (MapleInventoryManipulator.addFromDrop(c, item, true, true)) {
            ...
            if (c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(), 23000, Arrays.asList(MapleMapObjectType.HIRED_MERCHANT, MapleMapObjectType.SHOP)).size() == 0 && (c.getPlayer().getMapId() == 910000001 || c.getPlayer().getMapId() == 910000002 || c.getPlayer().getMapId() == 910000003)) {
                ...
                c.getPlayer().dropMessage(1, "You cannot open a merchant here.");
            }
            ...
            short bundles = slea.readShort(); // this is the total quantity !?!?!
            short perBundle = slea.readShort(); // this is actually the number of bundles
            int price = slea.readInt();
//            if (perBundle > 1) {
//                String txt = "Bundles are not supported yet.";
//                if (c.getPlayer().getGMLevel() > 2) {
//                    txt += "  bundles: " + bundles + "/ perBundle: " + perBundle;
//                }
...
//            }
            ...
            sellItem.setQuantity(perBundle); // Wait what? Quantity gets set to items in a bundle, not the total amount
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                if (ivItem != null && ivItem.getQuantity() >= bundles * perBundle) {
                    ...
                    if (bundles > 4000 || perBundle > 50) { // inverted this since it's fucked.
                        return;
                    }
                    MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                ...
                if (item.getBundles() > 0) {
                    ...
                    MapleInventoryManipulator.addFromDrop(c, iitem, true, true);
                }
            ...
            if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(c.getPlayer())) {
                ...
                for (MaplePlayerShopItem items : merchant.getItems()) {
                    if (items.getBundles() > 0) {
                        IItem item = items.getItem();
                        item.setQuantity(items.getBundles()); //***<<< Same here
                        if (MapleInventoryManipulator.addFromDrop(c, item, true, true)) {
                            items.setBundles((short) 0);
                        } else {
                            save = true;
