...
public class ChannelServer implements Runnable, ChannelServerMBean {
            ...
            synchronized (cwi) {
                ...
                synchronized (wci) {
                    ...
                    try {
                        ...
                        bossMesoRate = Integer.parseInt(props.getProperty("world.specialBossMesoRate"));
                        ...
                        serverName = props.getProperty("world.serverName");
                        godlyItems = Boolean.parseBoolean(props.getProperty("world.godlyItems", "false"));
                        itemStatMultiplier = Short.parseShort(props.getProperty("world.itemStatMultiplier"));
                        godlyItemRate = Short.parseShort(props.getProperty("world.godlyItemRate"));
                        ...
                        AB = Boolean.parseBoolean(props.getProperty("world.Autoban", "false"));
                        CS = Boolean.parseBoolean(props.getProperty("world.CashShop", "false"));
                        MT = Boolean.parseBoolean(props.getProperty("world.MTS", "false"));
                        GMItems = Boolean.parseBoolean(props.getProperty("world.GMItems", "false"));
                        extraCommands = Boolean.parseBoolean(props.getProperty("world.extraCommands", "false"));
                        PvPis = Integer.parseInt(props.getProperty("world.PvPis", "4"));
    ...
    public void run() {
        ...
        tMan.start();
        tMan.register(new SaveBot(), 300000, channel * 30000);
    ...
    public class SaveBot implements Runnable {

        private final WriteLock wl = new ReentrantReadWriteLock(true).writeLock();

        public void run() {
            if (!shutdown) {
                System.out.println("[SaveBot] Saving Channel " + channel);
                wl.lock();
                try {
                    for (final MapleCharacter p : players.getAllCharacters()) {
                        if (p != null) {
                            p.saveToDB(true, true);
                        }
                    }
                } finally {
                    wl.unlock();
