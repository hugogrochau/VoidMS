...
public class AbstractPlayerInteraction {
    ...
    public void gainItem(int id, short quantity, boolean randomStats) {
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !ii.isThrowingStar(item.getItemId()) && !ii.isBullet(item.getItemId())) {
                if (randomStats) {
                    MapleInventoryManipulator.addFromDrop(c, ii.randomizeStats((Equip) item), true, true);
                } else {
                    MapleInventoryManipulator.addFromDrop(c, (Equip) item, true, true);
                }
            } else {
    ...
    public void givePartyExp(int amount, List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            chr.gainExp(amount * c.getChannelServer().getExpRate() * (int) c.getPlayer().getExpRate(), true, true);
        }
    }
