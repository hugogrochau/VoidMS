...
public class PvPLibrary {
        ...
        for (int dmgpacket = 0; dmgpacket < attack.numDamage; dmgpacket++) {
            TimerManager.getInstance().schedule(new Runnable() {
                ...
                public void run() {
                    ...
                    int total = attackedPlayers.getTotalDex() + attackedPlayers.getTotalInt() + attackedPlayers.getTotalLuk() + attackedPlayers.getTotalStr();
                    pvpDamage /= (total / 1000);
                    /*
                    switch (Integer.parseInt(Character.toString(Integer.toString(attack.skill).charAt(0)))) {
                    case 9: // GM
                    case 1: // Warrior
                    pvpDamage *= (player.getTotalStr() / 1000);
                    break;
                    case 2: // Magician
                    pvpDamage *= (player.getTotalInt() / 1000);
                    break;
                    case 3: // Bowman
                    case 5: // Pirate
                    pvpDamage *= (player.getTotalDex() / 1000);
                    break;
                    case 4: // Rogue
                    pvpDamage *= (player.getTotalLuk() / 1000);
                    break;
                    }*/
                    // ISkill
                    ...
                    player.getMap().broadcastMessage(attackedPlayers, MaplePacketCreator.damagePlayer(attack.numDamage, pvpMob.getId(), attackedPlayers.getId(), pvpDamage, 0, 0, false, 0, false, 0, 0, 0), true);
                    //player.getMap().broadcastMessage(MaplePacketCreator.damagePlayer(attack.numDamage, pvpMob.getId(), attackedPlayers.getId(), pvpDamage));
                    ...
                    if (mesguard) {
                        player.getClient().getSession().write(MaplePacketCreator.serverNotice(5, attackedPlayers.getName() + " has partially blocked your attack with mesoguard!"));
                        mesguard = false;
                    }
                    if (magicrecovery) {
                        attackedPlayers.getClient().getSession().write(MaplePacketCreator.serverNotice(5, attackedPlayers.getName() + " has partially absorbed your MP with MP Eater!"));
                        magicrecovery = false;
                    }
                    // rewards
                    attackedPlayers.dropMessage("You just got hit by " + player.getName() + " with " + pvpDamage + " damage. HP Left: " + attackedPlayers.getHp() + "/" + attackedPlayers.getMaxHp());
                    player.dropMessage("You just hit " + attackedPlayers.getName() + " with " + pvpDamage + " damage. HP Left: " + attackedPlayers.getHp() + "/" + attackedPlayers.getMaxHp());
                    if (attackedPlayers.isDead() && attackedPlayers.getHp() <= 0) {
                        player.getMap().broadcastMessage(MaplePacketCreator.serverNotice(6, "[PVP] " + player.getName() + " has just killed " + attackedPlayers.getName() + " and now has " + player.getPvpKills() + " PvP kills."));
    ...
    public static void playerReward(MapleCharacter player, MapleCharacter attackedPlayers) {
        ...
        if (player.getPvpKills() > 0) {
            player.achieve(Achievement.FPVPK);
        } else if (player.getPvpKills() >= 100) {
            player.achieve(Achievement.PVP100);
        } else if (player.getPvpKills() >= 1000) {
            player.achieve(Achievement.PVP1000);
        }
        attackedPlayers.setPvpDeaths(attackedPlayers.getPvpDeaths() + 1);
        ...
        if (attackedPlayers.getMeso() < mesoReward) {
    ...
    public synchronized static void doPvP(MapleCharacter player, AbstractDealDamageHandler.AttackInfo attack) {
        ...
        getDirection(attack);
        if (player.isTagged() || player.getMap().isTagged()) {
            for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), PvPLibrary.maxDis, PvPLibrary.maxHeight, Collections.unmodifiableCollection(player.getMap().getCharacters()))) {
                if (!attackedPlayers.isHidden() && attackedPlayers.isAlive() && (player.getParty() == null || player.getParty() != attackedPlayers.getParty())) {
                    player.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, attackedPlayers.getName() + " just got owned by " + player.getName() + "!"));
                    attackedPlayers.setHp(0);
                    attackedPlayers.updateSingleStat(MapleStat.HP, 0);
                    attackedPlayers.getMap().broadcastMessage(attackedPlayers, MaplePacketCreator.showJobChange(attackedPlayers.getId()), true);
                }
            }
        } else if (player.getMapId() != 1010000) {
            for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), PvPLibrary.maxDis, PvPLibrary.maxHeight, Collections.unmodifiableCollection(player.getMap().getCharacters()))) {
                if (!attackedPlayers.isHidden() && attackedPlayers.isAlive() && (player.getParty() == null || player.getParty() != attackedPlayers.getParty())) {
                    if (player.getClient().getSession().getRemoteAddress() == attackedPlayers.getClient().getSession().getRemoteAddress()) {
                        player.dropMessage("You cannot attack your own characters");
                    } else {
            ...
            for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters())) {
                if (!attackedPlayers.isHidden() && attackedPlayers.isAlive() && (player.getParty() == null || player.getParty() != attackedPlayers.getParty())) {
                    if (attackedPlayers.getTimeShit() >= 5) {
                        if (attackedPlayers.isBlueTeam()) {
                            attackedPlayers.changeMap(1010000, 5);
                            player.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, attackedPlayers.getName() + " has been warped back for receiving five hits!"));
                            attackedPlayers.getClient().getSession().write(MaplePacketCreator.getClock(player.getMap().getMapTimedTimer()));
                            attackedPlayers.setTimeShit(0);
                        } else {
                            if (attackedPlayers.isRedTeam()) {
                                attackedPlayers.changeMap(1010000, 0);
                                player.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, attackedPlayers.getName() + " has been warped back for receiving five hits!"));
                                attackedPlayers.getClient().getSession().write(MaplePacketCreator.getClock(player.getMap().getMapTimedTimer()));
                                attackedPlayers.setTimeShit(0);
                            }
                    ...
                    int flagmap = 1010000;
                    if (flagmap == attackedPlayers.getMapId()) {
                        if (!attackedPlayers.hasFlag()) {// haveItem(4001125, 1, false, true)) {
                            attackedPlayers.setTimeShit(attackedPlayers.getTimeShit() + 1);
                        } else {
                            IItem itemid = new Item(4001025, (byte) 3, (short) 1);
                            attackedPlayers.getMap().spawnItemDrop(player, player, itemid, attackedPlayers.getPosition(), true, false);
                            MapleInventoryManipulator.removeAllById(attackedPlayers.getClient(), 4001025, true);
                            attackedPlayers.setHasFlag(false);
                            attackedPlayers.setTimeShit(attackedPlayers.getTimeShit() + 1);
                            attackedPlayers.getMap().broadcastMessage(MaplePacketCreator.serverNotice(5, player.getName() + " has just made " + attackedPlayers.getName() + " drop the flag!"));
                        }
                    }
                    monsterBomb(player, attackedPlayers, attack);
                }
