        ...
        if (map == null) {
            synchronized (this) {
                ...
                try {
                    ...
                    rs.close();
                    if (map.isHideout()) {
                        ps = con.prepareStatement("SELECT `npcid`, `x`, `y`, `fh` FROM `hideout_npcs` WHERE `mapid` = ?");
                        ps.setInt(1, omapid);
                        rs = ps.executeQuery();
                        while (rs.next()) {
                            ...
                            int y = rs.getInt("y");
                            int x = rs.getInt("x");
                            AbstractLoadedMapleLife life = loadLife(id, 0, false, fh, y, (x + 50), (x - 50), x, y, "n");
                    ...
                    if (npcs || !type.equals("n")) {
                        ...
                        if (myLife instanceof MapleMonster) {
                            ...
                            if (mobTime == -1 && respawns) { //does not respawn, force spawn once
                                map.spawnMonster(monster, false);
                            } else {
                                map.addMonsterSpawn(monster, mobTime);
                            }
