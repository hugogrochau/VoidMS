        ...
        if (overwrite) {
            for (MaplePlayerShopItem pItems : items) {
                if (pItems.getBundles() > 0) {
                    ...
                    ps.setInt(1, getOwnerId()); //Somewhere here needs Quantity
                    ps.executeUpdate();         //Maybe you might know how, Adding the bundle quantity
                    ps.close();
                }
            }
        }
    ...
    public boolean returnItems(MapleClient c) {
        for (MaplePlayerShopItem item : items) {
            if (item.getBundles() > 0) {
                ...
                if (MapleInventoryManipulator.addFromDrop(c, nItem, true)) {
