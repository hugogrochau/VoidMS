...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private void giveExpToCharacter(MapleCharacter attacker, int exp, boolean highestDamage, int numExpSharers) {
        ...
        if (attacker.getHp() > 0) {
            ...
            if (exp > 0) {
                ...
                if (attacker.getDonorLevel() == 1) {
                    personalExp *= 1.25;
                } else if (attacker.getDonorLevel() == 2) {
                    personalExp *= 1.75;
                }
                personalExp = (long) Math.round(personalExp);
            }
            if (personalExp > Integer.MAX_VALUE) {
    ...
    public MapleCharacter killBy(MapleCharacter killer) {
        long totalBaseExpL = this.getExp() * ChannelServer.getInstance(killer.getClient().getChannel()).getExpRate() * killer.getClient().getPlayer().hasEXPCard() * (long) killer.getExpRate();
