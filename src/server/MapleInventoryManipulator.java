...
public class MapleInventoryManipulator {
    ...
    public static boolean addRing(MapleCharacter chr, int itemId, int ringId) {
        ...
        byte newSlot = chr.getInventory(type).addItem(nEquip); // add to local inventory
        if (newSlot == -1) {
            return false;
        }
        chr.getClient().getSession().write(MaplePacketCreator.addInventorySlot(type, nEquip)); // send packet
        return true;
    }
...
//                if (itemId == 4002000 || itemId == 4002003 || itemId == 4002002 || itemId == 4031543 || itemId == 4031544 || itemId == 4031545 || itemId == 4031137 || itemId == 2340000 || itemId == 4000038 || itemId == 4001113 || itemId == 4001114 || itemId == 4031284) {
//                    for (ChannelServer cservs : ChannelServer.getAllInstances()) {
//                        for (MapleCharacter players : cservs.getPlayerStorage().getAllCharacters()) {
//                            if (players.getGMLevel() > 1 && players.inSpyMode()) {
//                                players.getClient().getSession().write(MaplePacketCreator.sendYellowTip(c.getPlayer().getName() + " has just gained " + quantity + " of " + itemId + " (" + MapleItemInformationProvider.getInstance().getItemName(itemId) + ")"));
//                            }
//                        }
//                    }
//                }
    ...
    public static boolean addFromDrop(MapleClient c, IItem item) {
        return addFromDrop(c, item, false);
    }
    ...
    public static boolean addFromDrop(MapleClient c, IItem item, boolean show, boolean trade) {
        return addFromDrop(c, item, show, null, trade);
    }

    public static boolean addFromDrop(MapleClient c, IItem item, boolean show, String owner, boolean trade) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
...
//                if (item.getItemId() == 4002000 || item.getItemId() == 4002003 || item.getItemId() == 4002002 || item.getItemId() == 4031543 || item.getItemId() == 4031544 || item.getItemId() == 4031545 || item.getItemId() == 4031137 || item.getItemId() == 2340000 || item.getItemId() == 4000038 || item.getItemId() == 4001113 || item.getItemId() == 4001114 || item.getItemId() == 4031284) {
//                    for (ChannelServer cservs : ChannelServer.getAllInstances()) {
//                        for (MapleCharacter players : cservs.getPlayerStorage().getAllCharacters()) {
//                            if (players.getGMLevel() > 1 && players.inSpyMode()) {
//                                players.getClient().getSession().write(MaplePacketCreator.sendYellowTip(c.getPlayer().getName() + " has just looted " + quantity + " of " + item.getItemId() + " (" + MapleItemInformationProvider.getInstance().getItemName(item.getItemId()) + ")"));
//                            }
//                        }
//                    }
//                }
            ...
            if (quantity == 1) {
                ...
                if (newSlot == -1) {
                    if (!c.getPlayer().getStorage().isFull() && trade) {
                        c.getPlayer().dropMessage("Lost item was sent to storage");
    ...
    public static void removeFromSlot(MapleClient c, MapleInventoryType type, byte slot, short quantity, boolean fromDrop, boolean consume) {
        ...
        if (item.getQuantity() == 0 && !allowZero) {
            ...
            if (item.getItemId() == 4031558 || item.getItemId() == 4002002) {
                c.getPlayer().cancelExpMultiplier();
            }
        } else {
    ...
    public static void removeById(MapleClient c, MapleInventoryType type, int itemId, int quantity, boolean fromDrop, boolean consume, int npcId) {
        ...
        if (remremove > 0) {
            throw new InventoryException("[h4x] " + c.getPlayer().getName() + " - Not enough items available (" + itemId + ", " + (quantity - remremove) + "/" + quantity + ") @ NPC: " + npcId);
        }
    }
    ...
    public static void removeById(MapleClient c, MapleInventoryType type, int itemId, int quantity, boolean fromDrop, boolean consume) {
        removeById(c, type, itemId, quantity, fromDrop, consume, 0);
    }
    ...
    public static void equip(MapleClient c, byte src, byte dst) {
        ...
        if (!c.getPlayer().isGM() && !c.getChannelServer().CanGMItem()) {
            switch (source.getItemId()) {
                ...
                case 1322013: // Wizet Secret Agent Suitcase
                    ...
                    c.getPlayer().dropMessage(1, "You're not a GM, therefore you cannot use this item.");
                    return;
            }
        }
        ...
        if (source.getRingId() > -1) {
            if (source.getItemId() > 1112015) {
                for (MapleRing ring : c.getPlayer().getFriendshipRings()) {
                    if (ring.equipped()) {
                        c.getPlayer().dropMessage(1, "You can't equip two rings at the same time");
                        return;
                    }
                }
            } else {
                for (MapleRing ring : c.getPlayer().getCrushRings()) {
                    if (ring.equipped()) {
                        c.getPlayer().dropMessage(1, "You can't equip two rings at the same time");
                        return;
                    }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, byte src, short quantity) {
        ...
        if (type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (ringid != -1) {
                try {
                    ...
                    ps.executeUpdate();
                    ps.close();
                    MapleInventoryManipulator.removeFromSlot(c, type, src, (short) 1, false);
                } catch (SQLException sqle) {
                }
                return;
            }
        }
        if (source.getItemId() == 4002001) {
            c.getPlayer().removeItem(4002001, c.getPlayer().getItemQuantity(4002001, false));
            if (c.getPlayer().getMapId() == c.getPlayer().getGuild().getHideout() && !c.getPlayer().getGuild().hideoutExpired()) {
                c.getPlayer().spawnHideoutNpc();
            }
            return;
        }
        if (c.getPlayer().getItemEffect() == itemId && source.getQuantity() == 1) {
            ...
            if (c.getPlayer().getItemQuantity(itemId, false) == 1) {
                if (c.getPlayer().getCanTalk() && !c.getPlayer().inEvent() && !c.getPlayer().getMap().isMuted()) {
                    c.getPlayer().setChalkboard(null);
                } else {
                    c.getPlayer().dropMessage("You can't use chalkboards if you're muted or in an Event");
                }
            }
        }
        ...
        if (quantity < source.getQuantity() && !ii.isThrowingStar(itemId) && !ii.isBullet(itemId)) {
            ...
            } else if (c.getPlayer().getMap().getEverlast() || c.getPlayer().getMapId() > 209000000 && c.getPlayer().getMapId() < 209000016) {
