...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			if (cm.getJob().equals(client.MapleJob.BEGINNER)) {
				...
				if (cm.getLevel() >= 30 
					&& cm.getJob().equals(client.MapleJob.MAGICIAN)) {
			...
			if (cm.getJob().equals(client.MapleJob.BEGINNER))
				cm.changeJob(client.MapleJob.MAGICIAN);
			...
			if (selection == 0) {
				...
				job = client.MapleJob.FP_WIZARD;
			} else if (selection == 1) {
				jobName = "Ice/Lightning Wizard";
				job = client.MapleJob.IL_WIZARD;
			} else {
				jobName = "Cleric";
				job = client.MapleJob.CLERIC;
			}
