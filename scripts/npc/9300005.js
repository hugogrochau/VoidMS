...
var GameDropProbability = 0.00001; 
var GameDrops = [ 
//ItemID, Drop Probability (Should add Up to 1) 
[4031543, 0.20],[4031544, 0.15],[4031545, 0.10], 
[4002000, 0.10],[4001013, 0.05],[2340000, 0.30] 
]; 
var Gems = [ 
//Ore, Gem 
...
function SendGameInstructions() { 
    cm.sendNext(Header() + 
        ...
        "      Destroyed Ores are worth 25 points each. Destroyed Gems are worth 50 points each. Special Items and Drop Items will not be destroyed.\r\n" + 
        "5. Real In-Game Items may appear. Click on them to get them:\r\n" + 
        "            #v4031543##v4031544##v4031544##v4002000##v4002003##v2340000#\r\n" + 
        "\r\nGood Luck ~" 
        ); 
} 
...
function DoMove(Move) { 
    ...
    RemoveAndGenerateNewGems(Combo, Move[0], Move[1], GemGroup); 
    return true; 
} 
...
function RemoveAndGenerateNewGems(Combo, r, c, GemGroup) { 
    var DropItemProb = GameDropProbability + ((Combo.length > 5) ? (GameDropProbability / 10) * (Combo.length - 5) : 0); 
    var SpecialItemProb = 0.001 + (Combo.length > 3 ? 0.005 * (Combo.length - 3) : 0.0) + DropItemProb; 
    for (var i = 0;i < Combo.length;i++) { 
        ...
        if (Combo[i][0] == r && Combo[i][1] == c && GemGroup != null 
            ...
            RandomOreGemDrop(NewGemLocation[0], NewGemLocation[1], DropItemProb, SpecialItemProb); 
        } 
    } 
} 
