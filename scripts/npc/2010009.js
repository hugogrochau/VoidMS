...
function action(mode, type, selection) {
	...
	if (status == 0)  {
		cm.sendOk("Alliances dont work yet.");
		cm.dispose();
	    ...
	    if (selection == 0) {
			...
			if (rank == 1, MapleAlliance.createAlliance(partymembers.get(0), partymembers.get(1), guildName) == null)
				cm.sendOk("Your party member doesnt have a guild, or you arent the Union Master");
			else {
				MapleAlliance.addGuild();
				cm.dispose();
			}
		} else if(selection == 4) {
		    var rank = cm.getPlayer().getMGC().getAllianceRank();
