...
function action(mode, type, selection) {
	...
	if (mode == -1) {
		...
		if (status >= 0 && mode == 0) {
			cm.sendOk("Ok, bye... Hope you can help me later");
			cm.dispose();
		}
		if (mode == 1)
			status ++;
			...
			status --;
		if (status == 0) {
			cm.sendNext("Hi #r#h #! As you most likely know, it is Valentines Day soon! There is this girl in my class, Kate, who I have a huge crush on! I want to try and impress her on Valentines Day.");
		} else if (status == 1) {
			...
			if (cm.itemQuantity(4031938) >= 30) && if (cm.itemQuantity(4031111) >= 1) {
			cm.gainItem(4031938, -30);
			cm.gainItem(4031111, -1);
			cm.sendOk("Thank you so much for retrieving my items, #r#h #. I made this box of chocolates with the items you gave me. I also wrote her a poem. I'm really shy around Kate, so could you go give it to her? She works at #bDuane Reads in NLC");
