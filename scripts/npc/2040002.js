...
function action(mode, type, selection) {
	if(mode == -1) {
		...
		if(cm.getQuestStatus(3230).getId() == client.MapleQuestStatus.Status.STARTED.getId()) {
			if(status == 0) {
		...
		} else if(cm.getQuestStatus(3230).getId() == client.MapleQuestStatus.Status.COMPLETED) {
