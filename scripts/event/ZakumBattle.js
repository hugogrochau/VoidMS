...
function playerDisconnected(eim,player) {
	...
	if (player.getName().equals(eim.getProperty("leader"))) {
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6,"The leader of the instance has disconnected."));
		}
	}
...
function monsterValue(eim,mobId) { // potentially display time of death? does not seem to work
	if (mobId == 8800002) { // 3rd body
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6,"Congratulations on defeating Zakum!"));
		}
	}
	return -1;
}
