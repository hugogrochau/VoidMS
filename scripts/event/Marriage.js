...
function playerEntry(eim, player) {
	...
	//player.getClient().getSession().write(tools.MaplePacketCreator.getClock(180));
...
function playerWarpAltar(eim, player) {
	...
	player.getClient().getSession().write(tools.MaplePacketCreator.getClock(300));
	}else{
	player.changeMap(altarMap, altarMap.getPortal(2));
	player.getClient().getSession().write(tools.MaplePacketCreator.getClock(300));
	player.getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6, "Please talk to High Priest John now!"));
	}
}
