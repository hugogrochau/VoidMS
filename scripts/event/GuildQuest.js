...
function begin(eim) {
		...
		var iter = party.iterator();
                while (iter.hasNext()) {
                        iter.next().getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6,"The quest has begun."));
		}
        //}
}
...
function playerDisconnected(eim, player) {
	...
	if (player.getName().equals(eim.getProperty("leader"))) { //check for party leader
		...
		var iter = party.iterator();
                while (iter.hasNext()) {
			var pl = iter.next();
                        pl.getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6,"The leader of the instance has disconnected, and the remaining players shall be warped out."));
			if (pl.equals(player)) {
				removePlayer(eim, pl);
			}			
			else {
...
function end(eim, msg) {
        ...
        while (iter.hasNext()) {
                ...
                player.getClient().getSession().write(tools.MaplePacketCreator.serverNotice(6,msg));
		eim.unregisterPlayer(player);
                player.changeMap(exitMap, exitMap.getPortal(0));
	}
	eim.dispose();
...
function clearPQ(eim) {
	var iter = eim.getPlayers().iterator();
		...
		player.changeMap(bonusMap, bonusMap.getPortal(0));
                player.getClient().getSession().write(tools.MaplePacketCreator.getClock(40));
	}
...
function earringcheck(eim, player) {
	var iter = eim.getPlayers().iterator();
		...
		var pl = iter.next();
			...
			pl.addHP(-30000);
			pl.getMap().broadcastMessage(tools.MaplePacketCreator.serverNotice(6,pl.getName() + " died from not wearing earrings!"));
                }
        }
        eim.schedule("earringcheck", 15000);
}
